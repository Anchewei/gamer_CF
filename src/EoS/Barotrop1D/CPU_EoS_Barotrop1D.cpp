...
double Max_Rho;
double dRho; 
double *Table_All = NULL;
double *Table_Rho = NULL;
double *Table_Temp = NULL;
...
2. This file is shared by both CPU and GPU
   ...
   GPU_EoS_Barotrop1D.cu -> CPU_EoS_Barotrop1D.cpp
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_SetAuxArray_Barotrop1D
// Description :  Set the auxiliary arrays AuxArray_Flt/Int[]
//
// Note        :  1. Invoked by EoS_Init_Barotrop1D()
...
//-------------------------------------------------------------------------------------------------------
#ifndef __CUDACC__
void EoS_SetAuxArray_Barotrop1D( double AuxArray_Flt[], int AuxArray_Int[] )
{

   AuxArray_Int[BAROTROP1D_AUX_NRHO] = nRho;

   AuxArray_Flt[BAROTROP1D_AUX_MAXRHO] = Min_Rho;
   AuxArray_Flt[BAROTROP1D_AUX_MINRHO] = Max_Rho;
   AuxArray_Flt[BAROTROP1D_AUX_DRHO] = dRho;
   AuxArray_Flt[BAROTROP1D_AUX_GAMMA] = GAMMA;
   AuxArray_Flt[BAROTROP1D_AUX_GAMMA_M1] = GAMMA - 1.0;
   AuxArray_Flt[BAROTROP1D_AUX__GAMMA_M1] = 1.0 / ( GAMMA - 1.0 );
   AuxArray_Flt[BAROTROP1D_AUX_DENS2CGS] = UNIT_D;
   AuxArray_Flt[BAROTROP1D_AUX_M_KB] = MOLECULAR_WEIGHT * Const_amu / Const_kB * (UNIT_E/UNIT_M);
   AuxArray_Flt[BAROTROP1D_AUX__M_KB] = 1.0 / ( MOLECULAR_WEIGHT * Const_amu / Const_kB * (UNIT_E/UNIT_M) );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_DensEint2Pres_Barotrop1D
...
//                2. See EoS_SetAuxArray_Barotrop1D() for the values stored in AuxArray_Flt/Int[]
//
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensEint2Pres_Barotrop1D( const real Dens, const real Eint, const real Passive[],
...
#  endif // GAMER_DEBUG
   ...
   const real Gamma_m1 = (real)AuxArray_Flt[BAROTROP1D_AUX_GAMMA_M1];
   real Pres;
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensPres2Eint_Barotrop1D( const real Dens, const real Pres, const real Passive[],
                                             const double AuxArray_Flt[], const int AuxArray_Int[],
                                             const real *const Table[EOS_NTABLE_MAX] )
{
...
#  endif // GAMER_DEBUG
   ...
   const real _Gamma_m1 = (real)AuxArray_Flt[BAROTROP1D_AUX__GAMMA_M1];
   real Eint;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_DensPres2CSqr_Barotrop1D
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensPres2CSqr_Barotrop1D( const real Dens, const real Pres, const real Passive[],
                                             const double AuxArray_Flt[], const int AuxArray_Int[],
                                             const real *const Table[EOS_NTABLE_MAX] )
{
...
#  endif // GAMER_DEBUG
   ...
   const real Gamma = (real)AuxArray_Flt[BAROTROP1D_AUX_GAMMA];
   const real Gamma_m1 = (real)AuxArray_Flt[BAROTROP1D_AUX_GAMMA_M1];
   const real _Gamma_m1 = (real)AuxArray_Flt[BAROTROP1D_AUX__GAMMA_M1];
   const real Eint = Pres * _Gamma_m1;
   real Cs2;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_DensEint2Temp_Barotrop1D
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensEint2Temp_Barotrop1D( const real Dens, const real Eint, const real Passive[],
                                             const double AuxArray_Flt[], const int AuxArray_Int[],
                                             const real *const Table[EOS_NTABLE_MAX] )
{
...
#  endif // GAMER_DEBUG
   ...
   const real MinRho = (real) AuxArray_Flt[BAROTROP1D_AUX_MAXRHO];
   const real MaxRho = (real) AuxArray_Flt[BAROTROP1D_AUX_MINRHO];
   const real DRho = (real) AuxArray_Flt[BAROTROP1D_AUX_DRHO];
   const real Dens2CGS = (real) AuxArray_Flt[BAROTROP1D_AUX_DENS2CGS];
   const real Dens_CGS = Dens * Dens2CGS;
   const real i = ( LOG10(Dens_CGS) - MinRho ) / DRho ;
   const int j = FLOOR(i);
   
   real Temp;
   Temp = ( i - j ) * Table[BAROTROP1D_TAB_TEMP][ j + 1 ] + (1.0 - (i - j) ) * Table[BAROTROP1D_TAB_TEMP][j];
   Temp = POW(10.0, Temp);
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_DensTemp2Pres_Barotrop1D
// Description :  Convert gas mass density and temperature to gas pressure
//
// Note        :  1. See EoS_SetAuxArray_Barotrop1D() for the values stored in AuxArray_Flt/Int[]
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensTemp2Pres_Barotrop1D( const real Dens, const real Temp, const real Passive[],
                                             const double AuxArray_Flt[], const int AuxArray_Int[],
                                             const real *const Table[EOS_NTABLE_MAX] )
{
...
#  endif // GAMER_DEBUG
   ...
   const real _m_kB = (real)AuxArray_Flt[BAROTROP1D_AUX__M_KB];
   real Pres;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_DensEint2Entr_Barotrop1D
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real EoS_DensEint2Entr_Barotrop1D( const real Dens, const real Eint, const real Passive[],
                                             const double AuxArray_Flt[], const int AuxArray_Int[],
                                             const real *const Table[EOS_NTABLE_MAX] )
{
...
//-------------------------------------------------------------------------------------------------------
// Function    :  EoS_General_Barotrop1D
...
//-------------------------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static void EoS_General_Barotrop1D( const int Mode, real Out[], const real In_Flt[], const int In_Int[],
                                       const double AuxArray_Flt[], const int AuxArray_Int[],
                                       const real *const Table[EOS_NTABLE_MAX] )
{
...
FUNC_SPACE EoS_DE2P_t EoS_DensEint2Pres_Ptr = EoS_DensEint2Pres_Barotrop1D;
FUNC_SPACE EoS_DP2E_t EoS_DensPres2Eint_Ptr = EoS_DensPres2Eint_Barotrop1D;
FUNC_SPACE EoS_DP2C_t EoS_DensPres2CSqr_Ptr = EoS_DensPres2CSqr_Barotrop1D;
FUNC_SPACE EoS_DE2T_t EoS_DensEint2Temp_Ptr = EoS_DensEint2Temp_Barotrop1D;
FUNC_SPACE EoS_DT2P_t EoS_DensTemp2Pres_Ptr = EoS_DensTemp2Pres_Barotrop1D;
FUNC_SPACE EoS_DE2S_t EoS_DensEint2Entr_Ptr = EoS_DensEint2Entr_Barotrop1D;
FUNC_SPACE EoS_GENE_t EoS_General_Ptr       = EoS_General_Barotrop1D;

//-----------------------------------------------------------------------------------------
// Function    :  EoS_SetCPU/GPUFunc_Barotrop1D
...
//
//                      EoS_SetFunc_Barotrop1D( CPU_FuncPtr, GPU_FuncPtr );
//
//                3. Call-by-reference
//
...
#ifdef __CUDACC__
__host__
void EoS_SetGPUFunc_Barotrop1D( EoS_DE2P_t &EoS_DensEint2Pres_GPUPtr,
...
void EoS_SetCPUFunc_Barotrop1D( EoS_DE2P_t &EoS_DensEint2Pres_CPUPtr,
...
void EoS_SetAuxArray_Barotrop1D( double [], int [] );
void EoS_SetCPUFunc_Barotrop1D( EoS_DE2P_t &, EoS_DP2E_t &, EoS_DP2C_t &, EoS_DE2T_t &, EoS_DT2P_t &, EoS_DE2S_t &, EoS_GENE_t & );
#ifdef GPU
void EoS_SetGPUFunc_Barotrop1D( EoS_DE2P_t &, EoS_DP2E_t &, EoS_DP2C_t &, EoS_DE2T_t &, EoS_DT2P_t &, EoS_DE2S_t &, EoS_GENE_t & );
#endif

//-----------------------------------------------------------------------------------------
// Function    :  EoS_Init_Barotrop1D
...
{
   Read_Barotrop1D_Table( ISM_Baratrop1D_Table );
   EoS_SetAuxArray_Barotrop1D( EoS_AuxArray_Flt, EoS_AuxArray_Int );
   EoS_SetCPUFunc_Barotrop1D( EoS_DensEint2Pres_CPUPtr, EoS_DensPres2Eint_CPUPtr,
                                 EoS_DensPres2CSqr_CPUPtr, EoS_DensEint2Temp_CPUPtr,
                                 EoS_DensTemp2Pres_CPUPtr, EoS_DensEint2Entr_CPUPtr,
                                 EoS_General_CPUPtr );
#  ifdef GPU
   EoS_SetGPUFunc_Barotrop1D( EoS_DensEint2Pres_GPUPtr, EoS_DensPres2Eint_GPUPtr,
